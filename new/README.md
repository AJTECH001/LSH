...
- Monitors unlimited users across any EVM chain simultaneously with zero polling overhead.
...
- Admin-managed hook address and authorized caller set for safe upgradeability.
...
- Users can `topUpDeposit()`, `updateThreshold()`, or `deactivateShield()` at any time.
...
cd liquidation_shield_hook/new
forge install
```
...
## 🚀 Deployed Contracts (Unichain Sepolia + Reactive Lasna)
...
See [DEPLOYMENT_PROCESS.md](DEPLOYMENT_PROCESS.md) for the full step-by-step guide and [DEPLOYMENT_LOGS.md](DEPLOYMENT_LOGS.md) for all verified transaction hashes.
